...
describe("Query Classifier", () => {
  ...
  describe("Heuristic Classifier", () => {
    it("should classify simple queries as direct mode", () => {
      const result = classifyQueryHeuristic("hello");
    ...
    it("should classify longer queries as retrieve mode", () => {
      const result = classifyQueryHeuristic("What is the difference between pgvector and traditional search?");
    ...
    it("should detect SQL indicators", () => {
      const result = classifyQueryHeuristic("SELECT count from documents WHERE title contains AI");
      expect(result.targets).toContain("sql");
      expect(result.targets).toContain("vector");
    });

    it("should detect recency indicators", () => {
      const result = classifyQueryHeuristic("What are the latest AI updates in 2025?");
    ...
    it("should deduplicate targets", () => {
      const result = classifyQueryHeuristic("latest today current news update recent");
  ...
  describe("LLM Classifier", () => {
    it("should classify queries when LLM mode is disabled", async () => {
      ...
      const result = await classifyQuery("What is vector search?");
    ...
    it("should handle LLM classification with mock", async () => {
      ...
      const result = await classifyQuery("What are the latest trends in RAG systems?");
    ...
    it("should fallback to heuristics on LLM failure", async () => {
      ...
      const result = await classifyQuery("test query for fallback");
    ...
    it("should handle greetings appropriately", async () => {
      process.env.USE_LLM_CLASSIFIER = "false";
      const result = await classifyQuery("Hi");
      expect(result.mode).toBe("direct");
      expect(result.complexity).toBe("low");
    });
    ...
    it("should identify database queries", async () => {
      process.env.USE_LLM_CLASSIFIER = "false";
      const result = await classifyQuery("How many documents are in the chunks table?");
      expect(result.targets).toContain("sql");
    });

    it("should identify time-sensitive queries", async () => {
      process.env.USE_LLM_CLASSIFIER = "false";
      const result = await classifyQuery("What happened yesterday in AI news?");
      expect(result.targets).toContain("web");
    });
  });
  ...
  describe("Target Selection", () => {
    it("should always include vector by default", () => {
      const result = classifyQueryHeuristic("random query");
      expect(result.targets).toContain("vector");
    });
    ...
    it("should handle year-specific queries", () => {
      const result = classifyQueryHeuristic("What were the AI breakthroughs in 2024?");
      expect(result.targets).toContain("web");
    });
  });
  ...
  describe("Complexity Assessment", () => {
    it("should rate short simple queries as low complexity", () => {
      const result = classifyQueryHeuristic("what is AI");
      expect(result.complexity).toBe("low");
    });

    it("should rate medium-length queries as medium complexity", () => {
      const result = classifyQueryHeuristic("explain how vector databases work");
      expect(result.complexity).toBe("medium");
    });

    it("should rate queries with operators as medium or high", () => {
      const result = classifyQueryHeuristic("compare vector search to keyword search");
  ...
  describe("Mode Selection", () => {
    it("should use direct mode for very short queries", () => {
      const result = classifyQueryHeuristic("hi");
      expect(result.mode).toBe("direct");
    });

    it("should use retrieve mode for knowledge queries", () => {
      const result = classifyQueryHeuristic("How do vector embeddings work in modern systems?");
      expect(result.mode).toBe("retrieve");
    });
    ...
    it("should use retrieve mode for queries with operators", () => {
      ...
      operators.forEach(op => {
        const result = classifyQueryHeuristic(`${op} does this work`);
        expect(result.mode).toBe("retrieve");
      });
    });
  });
